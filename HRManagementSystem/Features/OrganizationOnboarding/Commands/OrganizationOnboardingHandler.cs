...
namespace HRManagementSystem.Features.OrganizationOnboarding.Commands
{
    public sealed class OrganizationOnboardingHandler
        : RequestHandlerBase<OrganizationOnboardingCommand, RequestResult<bool>, Organization, int>
    {
        ...
        public override async Task<RequestResult<bool>> Handle(OrganizationOnboardingCommand request, CancellationToken ct)
        {
            ...
            if (!resultOrganization.isSuccess)
                return RequestResult<bool>.Failure(resultOrganization.message, resultOrganization.errorCode);
            #endregion

            #region 2. Loop through Companies
            foreach (var companyDto in request.OnboardingDto.Companies)
            {
                var companyAdded = await _mediator.Send(new AddCompanyCommand(
                                    companyDto.OrganizationId,
                                    companyDto.Name,
                                    companyDto.Code,
                                    companyDto.Description), ct);

                if (!companyAdded.isSuccess)
                    return RequestResult<bool>.Failure(companyAdded.message, companyAdded.errorCode);
                #endregion

                #region 3. Loop through Branches
                foreach (var branchDto in companyDto.Branches)
                {
                    var branchAddress = _mapper.Map<AddBranchAddressDto>(branchDto.Address);
                    var branchAdded = await _mediator.Send(new AddBranchCommand
                        (
                        branchDto.Name,
                    ...
                    if (!branchAdded.isSuccess)
                        return RequestResult<bool>.Failure(branchAdded.message, branchAdded.errorCode);
                    #endregion

                    #region 4. Loop through Departments
                    foreach (var deptDto in branchDto.Departments)
                    {
                        var deptAdded = await _mediator.Send(new AddDepartmentCommand(
                                        deptDto.BranchId,
                                        deptDto.Name,
                                        deptDto.Code,
                                        deptDto.Description), ct);

                        if (!deptAdded.isSuccess)
                            return RequestResult<bool>.Failure($"Department {deptDto.Name} wasn't added!", ErrorCode.InternalServerError);
                        #endregion

                        #region 5. Loop through Teams
                        foreach (var teamDto in deptDto.Teams)
                        {

                            var teamAdded = await _mediator.Send(new AddTeamCommand(
                                teamDto.Name,
                                teamDto.Code,
                                teamDto.Description,
                                teamDto.DepartmentId
                                ), ct);

                            if (!teamAdded.isSuccess)
                                return RequestResult<bool>.Failure($"Team {teamDto.Name} wasn't added!", ErrorCode.InternalServerError);
                        }
                        #endregion
                    }
            ...
            return RequestResult<bool>.Success(true, "Organization onboarding completed successfully!");
        }
    }
}
