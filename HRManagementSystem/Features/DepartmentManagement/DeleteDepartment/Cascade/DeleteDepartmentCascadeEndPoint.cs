...
public class DeleteDepartmentCascadeEndPoint : BaseEndPoint<DeleteEntityCascadeViewModel<Department, Guid>, ResponseViewModel<bool>>
{
    ...
    public async Task<ResponseViewModel<bool>> DeleteDepartmentCascade([FromQuery] DeleteEntityCascadeViewModel<Department, Guid> model, CancellationToken ct)
    {
        var validationResult = ValidateRequest(model);
        if (!validationResult.isSuccess)
            return ResponseViewModel<bool>.Failure(validationResult.errorCode);
        var isDeleted = await _mediator.Send(new DeleteEntityCascadeCommand<Department, Guid>(model.Id));
