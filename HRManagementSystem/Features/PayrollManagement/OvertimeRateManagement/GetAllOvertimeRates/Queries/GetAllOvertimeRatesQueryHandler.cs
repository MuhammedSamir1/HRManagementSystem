...
namespace HRManagementSystem.Features.PayrollManagement.OvertimeRateManagement.GetAllOvertimeRates.Queries
{
         ...
         RequestHandlerBase<GetAllOvertimeRatesQuery, RequestResult<PagedList<OvertimeRateDto>>, OvertimeRate, int>
    {
        ...
        {
            // 1. ضبط قيم الصفحات بشكل آمن
            var pageNumber = Math.Max(request.PageNumber, 1);
            var pageSize = Math.Clamp(request.PageSize, 1, MaxPageSize);

            // 2. ابدأ الاستعلام من الريبوزيتوري العام
            ...
            // 5. استدعاء الـ PagedList.CreateAsync الموجود عندك مع ProjectTo داخلياً
            var pagedList = await PagedList<OvertimeRateDto>.CreateAsync<OvertimeRate>(
                source: query,
            ...
            // 6. إعادة النتيجة
            return RequestResult<PagedList<OvertimeRateDto>>.Success(pagedList, "تم جلب قائمة معدلات العمل الإضافي بنجاح.");
        }

        // دالة مساعدة للفرز الآمن (حدود الحقول المسموح بها)
