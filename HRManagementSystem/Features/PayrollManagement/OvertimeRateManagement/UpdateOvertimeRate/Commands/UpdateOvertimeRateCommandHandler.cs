...
namespace HRManagementSystem.Features.PayrollManagement.OvertimeRateManagement.UpdateOvertimeRate.Commands
{
         ...
         RequestHandlerBase<UpdateOvertimeRateCommand, RequestResult<OvertimeRateDto>, OvertimeRate, int>
    {
        ...
        public override async Task<RequestResult<OvertimeRateDto>> Handle(UpdateOvertimeRateCommand request, CancellationToken ct)
        {
            ...
            {
                return RequestResult<OvertimeRateDto>.Failure("سعر العمل الإضافي غير موجود.", ErrorCode.NotFound);
            }
            ...
            if (isNameConflict)
            {
                return RequestResult<OvertimeRateDto>.Failure(
                    "هذا الاسم موجود بالفعل لسعر عمل إضافي آخر. يجب أن يكون فريداً.",
                    ErrorCode.Conflict);
            }
            ...
            //if (!isSaved)
            //{
            //    return RequestResult<OvertimeRateDto>.Failure("فشل في تحديث سعر العمل الإضافي.", ErrorCode.InternalServerError);
            //}

       
            var dto = _mapper.Map<OvertimeRateDto>(existingRate);

            return RequestResult<OvertimeRateDto>.Success(dto, "تم تحديث سعر العمل الإضافي بنجاح.");
        }
    }
}
